...
import {
  ...
  SetCurrentSiteAction,
  SetCurrentSiteGuidAction
...
export function appReducer(state: AppState = INITIAL_APP_STATE, action: Action): AppState {
  ...
  switch (action.type) {
    ...
    case AppActionTypes.SetSearchItemsTotalCount:
      ...
      break;
    case AppActionTypes.SetCurrentSite:
      newState = updateCurrentSite(state, action as SetCurrentSiteAction);
      break;
    case AppActionTypes.SetCurrentSiteGuid:
      newState = updateCurrentSiteGuid(state, action as SetCurrentSiteGuidAction);
      break;
    default:
      newState = { ...state };
  }
...
function updateUser(state: AppState, action: SetUserProfileAction): AppState {
  return { ...state, user: { ...action.payload, isGuest: state.user.isGuest } };
}
...
function updateCurrentSite(state: AppState, action: SetCurrentSiteAction) {
  const newState = { ...state };
  newState.site = action.payload;
  return newState;
}

function updateCurrentSiteGuid(state: AppState, action: SetCurrentSiteGuidAction) {
  const newState = { ...state };
  newState.site.siteGuid = action.payload;
  return newState;
}
